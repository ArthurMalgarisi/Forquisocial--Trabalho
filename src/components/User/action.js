import { USERS_CHANGE_CHILD_VALUE, USERS_CHANGE_VALUE, USERS_CHANGE_USERDATA_CHILD_VALUE, USER_LOGGED_OUT, USER_LOGGED_IN } from '../../constants/actionTypes';
...
export function changeVisibleDetailsDialog(show, data) {
    return dispatch => { 
        ...
        if (show) {
            dispatch({ type: USERS_CHANGE_VALUE, property: 'userData', payload: data })
        } else {
            setTimeout(() => { 
                dispatch({ type: USERS_CHANGE_VALUE, property: 'userData', payload: null }) 
            }, 250);
        }
    }
}
